...
kubectl apply -f k8s
kubectl set image deployments/server-deploypment server=bogeyduffer/multi-server:$SHA
kubectl set image deployments/client-deploypment client=bogeyduffer/multi-client:$SHA
kubectl set image deployments/worker-deploypment worker=bogeyduffer/multi-worker:$SHA
